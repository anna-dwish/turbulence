...
We evaluated our assumptions associated with this model and observed roughly even scatter about the 0 line in the residual plot, along with generally low Cook's Distance values, which can be found in the Appendix. 5-fold cross-validation yielded an average test error of `r round(cv.glm(turbulence, moment.1.mod.glm)$delta[1],3)` 
...
moment.2.mod <- glm(log(R_moment_2) ~ Sv + Re + St + St_2 + Sv*Re, data = turbulence)
pander(moment.2.mod, caption="Moment 2")
```

From the model output, we see that 
...
moment.3.mod <- glm(log(R_moment_3) ~ Sv_2 + Sv + Re, data=turbulence)
...
round(cv.glm(turbulence, moment.4.mod)$delta[1],3)
