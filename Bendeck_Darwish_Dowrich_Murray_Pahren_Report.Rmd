...
We evaluated our assumptions associated with this model and observed roughly even scatter about the 0 line in the residual plot, along with generally low Cook's Distance values. 5-fold cross-validation yielded an average test error of `r round(cv.glm(turbulence, moment.1.mod.glm)$delta[1],5)` 
...
From the model output, we see that as the Reynold's number increases, we expect the log of the second moment of our response to decrease. This suggests there is less variance in particule cluster volumes as the ratio between fluid velocity and body size increases. This effect can not be understood in isolation however, because of the interaction effect with the settling parameter, *Sv*. Despite the main effect of the settling parameter suggesting an increase in the log of the second moment of the response, this effect is reversed for higher levels of Reynold's number. Finally, there appears to be a quadratic relationship between the log of the second moment and the Stoke's number.

From 5-fold cross validation, we achieve an estimated test error of `r round(cv.glm(turbulence, moment.2.mod.glm)$delta[1],3)`. From the abbreviated table of predictions, we see that our uncertainity worsens for larger values of Stoke's number. These predictions reflect our model's results in that increasing Reynold's number generally lends to a lower estimate of the log of  the second moment, even if the settling parameter values increases.
...
For this model, interaction effects lended to higher estimated test errors and were not significant in terms of their p-values. For this reason, they are no interaction effects in our final model for predicting the log of the 3rd moment. From the model output, we see that as the Reynold's number increases, we expect the log of the third moment of our response to decrease. This suggests the "lopsideness" of the distribution of the log of the 3rd moment decreases the ratio between fluid velocity and body size increases [[3]][Bibliography]. Finally, there appears to be an increasing relationship in the log of the third moment and the *linear* effect of the settling parameter and a decreasing relationship with the *quadratic* effect of the settlign parameter. This suggests that there is a levelling off of the effect of the settling parameter as it increases.

From 5-fold cross validation, we achieve an estimated test error of `r round(cv.glm(turbulence, moment.3.mod.glm)$delta[1],3)`. From the abbreviated table of predictions, we see that our uncertainity slightly worsens for higher values of Reynold's number. These predictions reflect our model's results in that increasing Reynold's number generally lends to a lower estimate of the log of the third moment and generally increases for larger values of the settling parameter.
...
Holding all else constant, we expect the fourth moment for observations with a medium or high Reynolds number to be lower than that of observations with a low Reynolds number. For the baseline level (a low Reynolds number), increasing the settling parameter also increases the fourth moment. However, for medium and high Reynolds numbers, increasing the settling parameter does not lead to as large of an increase in the fourth moment, since the original settling parameter coefficient is reduced by the interactions. From 5-fold cross-validation we achieved an estimated test error of `r round(cv.glm(turbulence, moment.4.mod.glm)$delta[1],3)`.
...
Consistent with what we saw above, for constant settling parameter and Stokes numbers, we see that observations with high Reynolds numbers have lower fourth moments when compared to observatiosn with low Reynolds number. Additionally, the higher settling parameter led to an even higher prediction for the observation with a low Reynolds number compared to the observation with a high Reynolds number. However, it is worth noting that these predictions have very wide prediction intervals, indicating that it would be possible to see opposite scenarios occur in nature. 

# Conclusion

Our goals in this case study were to 1) model and interpret the relationship between *Reynolds number (Re)*, *Froude's Number (Fr)*, *Particle characteristic (St)*, and the *Settling Parameter (Sv)* and the first four moments of particle cluster volumes, which quantifies turbulence, and 2) estimate the test errors of our models. We accomplished this by first exploring various model types, interaction effects, and transformations. After, we chose our final models with cross validation and variable selection methods. Some of the key findings of our models is that the *Settling Parameter* served as a stronger predictor in place of *Froude's Number (Fr)* across all four models. Further, log transforming the the second, third, and fourth moments of particle cluster volumes lended much better to estimating a linear relationship with our covariates. Finally, for all moments except the third moment, the relationship between turbulence and the *Reynolds number (Re)* is dependent upon the value of the *Settling Parameter (Sv)*. For future studies, future models may wish to focus more directly on prediction to achieve a lower estimated test error, along with other values of the *Reynolds number (Re)* and *Froude's number (Fr)*.
